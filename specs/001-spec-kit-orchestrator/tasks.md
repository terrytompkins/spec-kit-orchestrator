...
- [x] T001 Create project structure per implementation plan in `src/orchestrator/` with subdirectories: `pages/`, `services/`, `models/`, `utils/`
- [x] T002 Create `requirements.txt` with dependencies: streamlit, pyyaml, python-dotenv, gitpython (optional), pytest, pytest-mock
- [x] T003 [P] Create `README.md` with project description, setup instructions, and local run documentation
- [x] T004 [P] Create `src/orchestrator/__init__.py` and `src/orchestrator/pages/__init__.py` and `src/orchestrator/services/__init__.py` and `src/orchestrator/models/__init__.py` and `src/orchestrator/utils/__init__.py`
- [x] T005 [P] Create `tests/` directory structure: `tests/unit/`, `tests/integration/`, `tests/fixtures/`
- [x] T006 [P] Create `pytest.ini` configuration file for test discovery
- [x] T007 Create `.specify/orchestrator/` directory structure for config and runs
...
- [x] T008 Create `src/orchestrator/utils/path_validation.py` with path allowlist validation and directory traversal prevention
- [x] T009 [P] Create `src/orchestrator/utils/secret_masker.py` with secret masking logic for logs and UI displays
- [x] T010 [P] Create `src/orchestrator/utils/yaml_parser.py` with YAML parsing and generation utilities
- [x] T011 Create `src/orchestrator/services/config_manager.py` to load admin configuration from `.specify/orchestrator/config.yml`
- [x] T012 Create `src/orchestrator/services/cli_executor.py` with CLI execution wrapper that streams stdout/stderr and captures exit codes
- [x] T013 Create `src/orchestrator/services/run_metadata.py` to create and manage execution metadata JSON files in `.specify/orchestrator/runs/<timestamp>/`
- [x] T014 Create `src/orchestrator/models/project.py` with Project entity dataclass
- [x] T015 [P] Create `src/orchestrator/models/phase_execution.py` with PhaseExecution entity dataclass
- [x] T016 [P] Create `src/orchestrator/models/artifact.py` with Artifact entity dataclass
- [x] T017 [P] Create `src/orchestrator/models/workspace_config.py` with WorkspaceConfiguration entity dataclass
- [x] T018 Create `src/orchestrator/app.py` as main Streamlit app entry point with basic navigation setup
...
- [x] T019 [P] [US1] Unit test for path validation in `tests/unit/test_path_validation.py` (valid paths, directory traversal attempts, allowlist checking)
- [x] T020 [P] [US1] Unit test for CLI executor in `tests/unit/test_cli_executor.py` (command execution, output streaming, exit code capture)
...
- [x] T022 [US1] Implement project creation form UI in `src/orchestrator/pages/project_creation.py` with fields: project name, parent directory, AI agent dropdown, optional GitHub token, extra params
- [x] T023 [US1] Add path validation to project creation form using `src/orchestrator/utils/path_validation.py` (validate against admin-configured base directory, prevent directory traversal)
- [x] T024 [US1] Add AI agent value validation in `src/orchestrator/pages/project_creation.py` (check against admin-configured allowed values)
- [x] T025 [US1] Implement `specify init` command execution in `src/orchestrator/pages/project_creation.py` using `src/orchestrator/services/cli_executor.py`
- [x] T026 [US1] Implement real-time output streaming in `src/orchestrator/pages/project_creation.py` using Streamlit's `st.empty()` container with incremental updates
- [x] T027 [US1] Add error handling in `src/orchestrator/pages/project_creation.py` for CLI not found, permission errors, invalid paths, command failures with user-friendly messages
- [x] T028 [US1] Create execution metadata record in `src/orchestrator/pages/project_creation.py` using `src/orchestrator/services/run_metadata.py` after `specify init` completes
- [x] T029 [US1] Add success/failure UI feedback in `src/orchestrator/pages/project_creation.py` (success message with project link, error display with retry option)
...
- [x] T033 [US3] Implement phase runner UI page in `src/orchestrator/pages/phase_runner.py` with buttons for Constitution, Specify, Clarify, Plan, Tasks, Analyze phases
- [x] T034 [US3] Add phase status indicators in `src/orchestrator/pages/phase_runner.py` (not started / in progress / completed / failed) for each phase
- [x] T035 [US3] Implement phase dependency checking in `src/orchestrator/pages/phase_runner.py` (check required artifacts exist, warn if running out of order)
- [x] T036 [US3] Add confirmation dialog in `src/orchestrator/pages/phase_runner.py` when user attempts to run phase out of order (warn but allow if confirmed)
- [x] T037 [US3] Implement phase command execution in `src/orchestrator/pages/phase_runner.py` using `src/orchestrator/services/cli_executor.py` for each phase
- [x] T038 [US3] Implement real-time output streaming in `src/orchestrator/pages/phase_runner.py` for phase execution (stream stdout/stderr to UI)
- [x] T039 [US3] Create execution metadata record in `src/orchestrator/pages/phase_runner.py` using `src/orchestrator/services/run_metadata.py` after each phase completes
- [x] T040 [US3] Add artifact linking in `src/orchestrator/pages/phase_runner.py` (show links to generated artifacts after successful execution)
- [x] T041 [US3] Implement rerun capability in `src/orchestrator/pages/phase_runner.py` (create new execution record with new timestamp, preserve previous history)
- [x] T042 [US3] Add error handling in `src/orchestrator/pages/phase_runner.py` (highlight errors in UI, preserve error output in execution log, allow retry)
- [x] T043 [US3] Disable Analyze button until Tasks completes in `src/orchestrator/pages/phase_runner.py` (enforce phase ordering)
...
- [x] T046 [US2] Implement interview chat UI in `src/orchestrator/pages/interview_chat.py` using Streamlit chat components (`st.chat_message()`)
- [x] T047 [US2] Create parameter document generator service in `src/orchestrator/services/parameter_generator.py` to generate both Markdown and YAML formats
- [x] T048 [US2] Implement backup creation in `src/orchestrator/services/parameter_generator.py` (create timestamped backup before overwriting existing documents)
- [x] T049 [US2] Add parameter document validation in `src/orchestrator/services/parameter_generator.py` (ensure all phases included, validate YAML structure)
- [x] T050 [US2] Integrate parameter generator with interview chat in `src/orchestrator/pages/interview_chat.py` (generate documents after chat completion)
- [x] T051 [US2] Add document preview in `src/orchestrator/pages/interview_chat.py` (show generated documents in expandable sections)
- [x] T052 [US2] Add regeneration capability in `src/orchestrator/pages/interview_chat.py` (allow user to regenerate parameters if needed)
...
- [x] T055 [US4] Create artifact reader service in `src/orchestrator/services/artifact_reader.py` to discover artifacts in predictable locations (`.specify/memory/constitution.md`, `specs/*/spec.md`, etc.)
- [x] T056 [US4] Implement artifact browser UI page in `src/orchestrator/pages/artifact_browser.py` with artifact list (type, name, timestamp, status)
- [x] T057 [US4] Add Markdown rendering in `src/orchestrator/pages/artifact_browser.py` using Streamlit's `st.markdown()` with proper formatting and syntax highlighting
- [x] T058 [US4] Implement artifact navigation in `src/orchestrator/pages/artifact_browser.py` (links between related artifacts: spec → plan → tasks)
- [x] T059 [US4] Add execution log viewer in `src/orchestrator/pages/artifact_browser.py` (display chronological list of phase executions with timestamps, status, links to detailed logs)
- [x] T060 [US4] Add "not generated yet" messaging in `src/orchestrator/pages/artifact_browser.py` (clear message for missing artifacts, suggest which phase needs to be run)
- [x] T061 [US4] Implement log file reading in `src/orchestrator/services/artifact_reader.py` (read stdout/stderr logs from execution metadata directories)
...
- [x] T064 [US5] Implement project discovery service in `src/orchestrator/services/project_discovery.py` to scan admin-configured base workspace directory for folders containing `.specify/` directories
- [x] T065 [US5] Create project selection UI page in `src/orchestrator/pages/project_selection.py` with project list (name, path, last modified timestamp)
- [x] T066 [US5] Add project selection handling in `src/orchestrator/pages/project_selection.py` (load project context, display current state: phases run, artifact status)
- [x] T067 [US5] Implement project validation in `src/orchestrator/services/project_discovery.py` (check project still exists, refresh list if moved/deleted)
- [x] T068 [US5] Add project state loading in `src/orchestrator/services/project_discovery.py` (determine which phases have been run, artifact status)
- [x] T069 [US5] Integrate project selection with main app navigation in `src/orchestrator/app.py` (set selected project in session state, navigate to project-specific pages)
